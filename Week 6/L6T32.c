...
LASKU* laskuri(int iLuku, LASKU *pAlku, bool *ekaTarkastus){
    ...
    pUusi = dynaaminen();
    if (*ekaTarkastus){ // use * to dereference the bool pointer
        pUusi->iEka = iLuku2; // use pAlku to get the last sum
    } else {
        pUusi->iEka = iLuku;
        *ekaTarkastus = true;
    }
    ...
    scanf("%d", &pUusi->iToka);
    pUusi->iYhteen = pUusi->iEka + pUusi->iToka; // calculate the sum
    sprintf(pUusi->aLaskutoimitus, "%i+%i=%i", pUusi->iEka, pUusi->iToka, pUusi->iYhteen);
    printf("\n%s\n", pUusi->aLaskutoimitus);
